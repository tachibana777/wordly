import type { Word, ProgressState } from '../types';
export const study = {
    pool(words: Word[], category: string, reviewOnly: boolean, records: ProgressState['records']) {
        return words.filter(word => (category === 'all' || word.category === category)
            && (!reviewOnly || records[word.id]?.status === 'practice'));
    },
    random(words: Word[], previousId?: string | null, random = Math.random) {
    ...
    counts(words: Word[], records: ProgressState['records']) {
    ...
    recentDays(activity: ProgressState['activity'], now = new Date()) {
    ...
    streak(activity: ProgressState['activity'], now = new Date()) {
