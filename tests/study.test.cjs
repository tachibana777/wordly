...
function load(saved, blocked = false) {
    const entries = new Map(saved === undefined ? [] : [['wordly.progress.v1', saved]]);
    const context = { window: {}, Date, localStorage: {
        getItem(key) { if (blocked) throw new Error('Storage disabled'); return entries.get(key) ?? null; },
        setItem(key, value) { if (blocked) throw new Error('Storage disabled'); entries.set(key, value); }
    } };
    context.window = context;
    vm.createContext(context);
    const run = file => vm.runInContext(fs.readFileSync(path.join(root, file), 'utf8'), context, { filename: file });
    run('src/data/categories.js');
    context.EngVocab.categories.forEach(category => run(`src/data/${category.id}.js`));
    run('src/data/translations.js'); run('src/data/grading.js'); run('src/lib/answers.js');
    run('src/lib/study.js'); run('src/lib/storage.js');
    return { ...context.EngVocab, entries };
}
...
test('400 complete unique vocabulary entries, exactly 50 per category', () => {
    ...
    for (const word of vocabulary) {
        for (const field of ['word', 'partOfSpeech', 'meaning', 'hint', 'example']) assert.ok(typeof word[field] === 'string' && word[field].length > 0, `${word.id}.${field}`);
...
test('global/category/review pools use current ratings', () => {
    ...
    const records = { 'office-1': { status: 'practice' }, 'office-2': { status: 'known' }, 'dev-1': { status: 'practice' } };
...
test('latest answer changes status without inflating known total; activity counts each rated attempt', () => {
    ...
    storage.rate('invalid', 'known'); storage.rate('office-1', 'invalid');
    assert.equal(storage.state.activity[study.dayKey()], 3);
...
test('every local asset in the entry HTML exists', () => {
    const html = fs.readFileSync(path.join(root, 'index.html'), 'utf8');
    for (const [, file] of html.matchAll(/(?:src|href)="((?:src|assets)\/[^\"]+)"/g)) assert.ok(fs.existsSync(path.join(root, file)), file);
...
test('answer matching accepts explicit synonyms and formatting, not substrings or negation', () => {
    const { vocabulary, answers } = load();
    const word = english => vocabulary.find(item => item.word === english);
...
test('natural wrappers and context-sensitive corrections retain meaning distinctions', () => {
    const { vocabulary, answers } = load();
    const check = (english, value) => answers.check(vocabulary.find(word => word.word === english), value).score;
...
test('all 400 profiles cover full and partial answers across all eight categories', () => {
    const { vocabulary, categories, answers, gradingCoverage } = load();
    assert.equal(gradingCoverage.size, 400);
...
test('Thai sara am and grammatical wrappers preserve meaningful words', () => {
    const { vocabulary, answers } = load();
    const check = (term, value) => answers.check(vocabulary.find(word => word.word === term), value).score;
